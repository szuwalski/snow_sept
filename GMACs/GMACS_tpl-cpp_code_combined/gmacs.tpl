...
//    are only useful for specifying a non-default pinfile name). 
...
// Commandline options
// ************************************************************************************ //
// -testing : run testing section
// -sim : simulate data
// -jitter seed : jitter initial parameter values (if seed=0, RNG is initialized using current time)
//                 overrides gmacs.dat setting
// -i : print info
// -retro x : do retrospective run with 'x' peels
// -verbose : set verbose level (overrides gmacs.dat setting)
// -ainp fn : use pin file fn (or gmacs.pin if fn is missing)
// -apin fn : use pin file fn (or gmacs.pin if fn is missing)
// -pin fn : use pin file fn (or gmacs.pin if fn is missing)
// -stopAfterFnCall x: stop after "x" objective function calls
// -verbose x: print out with verbose level "x"
// ************************************************************************************ //
DATA_SECTION

 !! TheHeader = adstring("## GMACS Version 2.20.34-combined; ** AEP **; Compiled 2026-01-15");
...
//-------------------------------
 ...
 END_CALCS
  ...
  int nlikes;                                              ///> number of likelihood components
 ...
 END_CALCS
  ...
  ivector nFparams(1,nfleet);                     ///> The number of deviations required for each fleet, by fleet
  ivector nYparams(1,nfleet);                     ///> The number of deviations for female Fs, by fleet
 ...
 END_CALCS
  ...
  vector rai_id(1,nSurveyRows); //--relative abundance index ids
 LOC_CALCS
 ...
 END_CALCS
  ...
  int nGrowthObsP; //--number of non-tagging growth observations (or 1)
...
// ================================================================================================
  ...
  init_int n_env_treat;
  !! echoinput << n_env_treat << endl;
  !! gmacs_ctl << n_env_treat << endl;
  ivector Env_link_nums(1,n_env_treat+1);
  !! if (n_env_treat>0)
  !! {
  ...
  3darray Env_specs(1,n_env_treat+1,1,max_env_vars+1,1,5);
  matrix Env_vars_used(syr,nyr,1,Num_env_vars+1);
  int Env_point;
  number ave_env_scale;
  number sd_env_scale;
  number num_env_scale;
  ivector N_env_par(1,n_env_treat)
  ...
  init_vector model_controls(1,19);
  ...
  int recZ_flag;                                           ///> recruitment size distribution option (0: standard way; 1: Tanner crab approach)
  int IsTyler;                                             ///> Kluge for "2 area" model
 ...
 LOC_CALCS
  {
    ...
    gmacs_ctl << setw(4) << setfixed()<< int(model_controls(19)) << " # Set to 1 to copy growth from males to females)" << endl;
    gmacs_ctl << endl;
    ...
    echoinput << model_controls(19) << " # Set to 1 to copy growth from males to females)" << endl;
    echoinput << endl;
          ...
          for ( int o = 1; o <= nshell; o++ ){
            ...
            for ( int l = 1; l <= nclass; l++ ) {
              ...
              if ( bInitializeUnfished == EQMDEVS )
                anystring = "Init_logdev_for_sex_"+sexes(h)+"_mature_"+maturestate(m)+"_"+shellstate(o)+"shell_class_"+str(l);
              else
 ...
 END_CALCS
  ...
  // Storage by year
  imatrix ThetaPnt(1,ntheta*Nyears2,1,2*nclass+7+1);

  // Storage by year
  matrix Tpar_control(1,1000,1,15);
  imatrix TtoIG(1,1000,1,4);

  int n_Tpar;
  ivector nparTs(1,ntheta)
  ivector Tpar_block(1,ntheta);
  vector Tpar_RW_sigma(1,ntheta);
  
  ivector T_block(1,ntheta);
  ivector T_block_fn(1,ntheta);
  ivector T_env_link(1,ntheta);
  ivector T_env_var(1,ntheta);
  ivector T_RW(1,ntheta);
  ivector T_RW_blk(1,ntheta);
  vector T_RW_sigma(1,ntheta);
 ...
 END_CALCS
  ...
  imatrix iYrsThetaChanges(1,ntheta,syr,nyrRetro+1);
  int ntheta_use;
  ivector Theta_patterns(1,1000);
  ivector Theta_patterns2(1,1000);
  ivector nThetaVaries(1,ntheta);
  int maxThetaVaries;
  int nTheta_patterns;
 ...
 END_CALCS
  ...
  vector T_ival(1,n_Tpar);
  vector T_lb(1,n_Tpar);
  vector T_ub(1,n_Tpar);
  ivector T_phz(1,n_Tpar);
  ivector prior_Ttype(1,n_Tpar);
  vector prior_T_p1(1,n_Tpar);
  vector prior_T_p2(1,n_Tpar);
  ivector T_relative(1,n_Tpar);
 ...
 LOC_CALCS
  ...
  if ( bInitializeUnfished == EQMDEVS && T_phz(1) > 0 ){
 ...
 END_CALCS
  ...
  // Storage by year
  imatrix GrowPnt(1,3*nsex*Nyears,1,2*nclass+5+1);
 ...
 END_CALCS;
  ...
  // Storage by year
  imatrix SelPnt(1,2*nsex*nfleet*Nyears2,1,2*nclass+7+1);
 ...
 END_CALCS
  ...
  ivector q_anal(1,nSurveys);         //--column 1
  vector cpue_lambda(1,nSurveys);     //--column 2
  vector cpue_emphasis(1,nSurveys);   //--column 3
  ivector q_mirror(1,nSurveys);       //--column 4
  ivector q_block(1,nSurveys);        //--column 5
  ivector q_block_fn(1,nSurveys); 
  !! q_block_fn.initialize(); // Not implemented
  ivector q_relative(1,nSurveys); 
  !! q_relative.initialize(); // Not implemented
  ivector q_env_var(1,nSurveys);      //--column 6
  ivector q_env_link(1,nSurveys);     //--column 7
  ivector q_RW(1,nSurveys);           //--column 8
  ivector q_RW_blk(1,nSurveys);       //--column 9
  vector q_RW_sigma(1,nSurveys);      //--column 10
  int n_qpar;
 LOC_CALCS
  ...
  ECHO(n_deviations)
 END_CALCS
  imatrix qToSurv(1,n_qpar,1,4);//--map from q index to non-mirrored survey index
  ivector qType(1,n_qpar);  //--"0" is block, "1" if environmental link
 ...
 END_CALCS
  ...
  ivector add_cv_block_fn(1,nSurveys); 
  !! add_cv_block_fn.initialize(); // Not implemented
  ivector add_cv_relative(1,nSurveys); 
   ...
   if (add_cv_mirror(ii)==0)
    if (add_cv_env_link(ii)!=0)
      ...
      for (int ienv_par=1;ienv_par<=N_env_par(add_cv_env_var(ii));ienv_par++)
       {
        PPnt += 1;
//	parname1(PPnt) = "q_add" + anystring + "_env_link" + "_par_" + str(ienv_par);
       }
      }
  ...
  ECHO(n_deviations);
 END_CALCS
  imatrix AddcvToSurv(1,n_addcv_par,1,4);
...
//-------------------------------
 ...
 END_CALCS
  ...
  // rbeta     = theta(5)
  // ra        = theta(6)  //--females
  // rbeta     = theta(7)  //--females
  // logSigmaR = theta(8+)
  // steepness = theta(9)
  // rho       = theta(10)
  // logN0     = theta(11,...)
  init_bounded_number_vector T_pars_est(1,n_Tpar,T_lb,T_ub,T_phz);
  !!ECHO(T_pars_est);
  // growth parameters
  ...
  !!ECHO(nYparams);
  !!ECHO(foffdevs_phz);
  init_bounded_vector_vector log_fdov(1,nfleet,1,nYparams,-10,10,foffdevs_phz);    ///> log-scale Female F offset devs to Male F + Female F offset
...
// --------------------------------------------------------------------------------------------------
  ...
  vector logRbar(syr,nyrRetro+1);                          ///> logarithm of average recruits(syr+1,nyr)
  ...
  matrix alpha(1,nsex,1,maxSizeIncVaries);                 ///> intercept for linear growth increment model (or mean postmolt size at alpha0 for PWRLAW_GROWTHMODELALT)
  matrix alpha0(1,nsex,1,maxSizeIncVaries);                ///> reference (small) pre-molt size yielding mean postmolt size beta for PWRLAW_GROWTHMODELALT model
  matrix beta(1,nsex,1,maxSizeIncVaries);                  ///> slope for the linear growth increment model (or mean postmolt size at beta0 for PWRLAW_GROWTHMODELALT)
  matrix beta0(1,nsex,1,maxSizeIncVaries);                 ///> reference (large) pre-molt size yielding mean postmolt size beta for PWRLAW_GROWTHMODELALT model
  ...
  matrix T_pars(1,1000);                                   ///> Extracted initial parameters
  ...
  matrix recruits(1,nsex,syr,nyrRetro);                    ///> vector of estimated recruits, by sex
  ...
  matrix pre_catch_out(1,nCatchDF,syr,nyrRetro);           ///> Predicted catch for output       WTS note: calculated but never used or exported
  matrix res_catch_out(1,nCatchDF,syr,nyrRetro);           ///> Residuals for output             WTS note: calculated but never used or exported
  vector log_q_catch(1,nCatchDF);                          ///> Catchability
  4darray pre_capabd(syr,nyrRetro,1,nfleet,1,nsex,1,nclass);///> predicted fishery capture abundance for output (summed over maturity state, shell condition)
  4darray pre_capbio(syr,nyrRetro,1,nfleet,1,nsex,1,nclass);///> predicted fishery capture biomass for output (summed over maturity state, shell condition)
  ...
  4darray d4_N_init(1,n_grp,1,NyrEquil,1,nseason,1,nclass);         ///> N matrix for equilibrium calculations
  ...
  // predictions for non-tagging growth observations
  vector prdPstMltSizes(1,nGrowthObsP); //--predicted postmolt sizes 
  vector nllGrowth(1,nGrowthObsP);      //--nlls for growth 
  number grwthPosFun;                   //--penalty for negative mean growth
    ...
    for (int ipar=1;ipar<=n_Tpar;ipar++)
     {
      ...
      T_pars_est(ipar) = GenJitter(IsJittered,IsPin,T_ival(ipar),T_lb(ipar),T_ub(ipar),T_phz(ipar),sdJitter,rands,randu,T_pars_est(ipar));
     }
    for (int ipar=1;ipar<=n_Gpar;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      G_pars_est(ipar) = GenJitter(IsJittered,IsPin,G_ival(ipar),G_lb(ipar),G_ub(ipar),G_phz(ipar),sdJitter,rands,randu,G_pars_est(ipar));
     }
    for (int ipar=1;ipar<=n_Spar;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      S_pars_est(ipar) = GenJitter(IsJittered,IsPin,S_ival(ipar),S_lb(ipar),S_ub(ipar),S_phz(ipar),sdJitter,rands,randu,S_pars_est(ipar));
     }
     ...
     for (int jpar=1;jpar<=nFparams(ipar); jpar++)
      {
       rands.fill_randn(rng2);
       randu.fill_randn(rng2);
       log_fdev(ipar,jpar) = GenJitter(IsJittered,IsPin,(fdev_ub(ipar)+fdev_lb(ipar))/2.0,fdev_lb(ipar),fdev_ub(ipar),f_phz(ipar),sdJitter,rands,randu,log_fdev(ipar,jpar));
      }
    for (int ipar=1;ipar<=nfleet;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      log_foff(ipar) = GenJitter(IsJittered,IsPin,(foff_ub(ipar)+foff_lb(ipar))/2.0,foff_lb(ipar),foff_ub(ipar),foff_phz(ipar),sdJitter,rands,randu,log_foff(ipar));
     }
    for (int ipar=1;ipar<=nfleet;ipar++)
     for (int jpar=1;jpar<=nYparams(ipar); jpar++)
      {
       rands.fill_randn(rng2);
       randu.fill_randn(rng2);
       log_fdov(ipar,jpar) = GenJitter(IsJittered,IsPin,0.0,-10.0,10.0,foffdevs_phz(ipar),sdJitter,rands,randu,log_fdov(ipar,jpar));
      }
    for (int ipar=rdv_syr;ipar<=rdv_eyr;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      rec_dev_est(ipar) = GenJitter(IsJittered,IsPin,0.0,-8.0,8.0,rdv_phz,sdJitter,rands,randu,rec_dev_est(ipar));
     }
    for (int ipar=rdv_syr;ipar<=rdv_eyr;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      logit_rec_prop_est(ipar) = GenJitter(IsJittered,IsPin,init_sex_ratio,-100.0,100.0,rec_prop_phz,sdJitter,rands,randu,logit_rec_prop_est(ipar));
     }
    for (int ipar=1;ipar<=nSizeComps;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      log_vn(ipar) = GenJitter(IsJittered,IsPin,log_vn_ival(ipar),log_vn_lb(ipar),log_vn_ub(ipar),log_vn_phz(ipar),sdJitter,rands,randu,log_vn(ipar));
     }
    for (int ipar=1;ipar<=n_qpar;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      survey_q(ipar) = GenJitter(IsJittered,IsPin,q_ival(ipar),q_lb(ipar),q_ub(ipar),q_phz(ipar),sdJitter,rands,randu,survey_q(ipar));
     }
    for (int ipar=1;ipar<=n_addcv_par;ipar++)
     {
      rands.fill_randn(rng2);
      randu.fill_randn(rng2);
      log_add_cv(ipar) = GenJitter(IsJittered,IsPin,log_add_cv_ival(ipar),log_add_cv_lb(ipar),log_add_cv_ub(ipar),cv_phz(ipar),sdJitter,rands,randu,log_add_cv(ipar));
     }
   }
...
FUNCTION dvar_vector timevarparV2(const int itype, const int ipar, const int lyear,  const int isrel, const int blk, const int block_fn, 
                                const int env_link, const int env_var,
                                const int RW, const int RW_blk, 
...
FUNCTION dvar_vector calc_growth_increments_linear(const dvector vSizes, const ivector iSex, int igrow)
...
FUNCTION dvar_vector calc_like_gamma_linear_growth(const dvector dMoltInc, const dvector vSizes, const ivector iSex, int igrow)
  if ( vSizes.indexmin() != iSex.indexmin() || vSizes.indexmax() != iSex.indexmax() )
    { cerr << "indices don't match..." << endl; ad_exit(1); }
  RETURN_ARRAYS_INCREMENT();
...
FUNCTION dvar_vector calc_like_gamma_pwrlaw_growth(const dvector dMoltInc, const dvector vSizes, const ivector iSex, int igrow)
  if ( vSizes.indexmin() != iSex.indexmin() || vSizes.indexmax() != iSex.indexmax() )
    { cerr << "indices don't match..." << endl; ad_exit(1); }
  RETURN_ARRAYS_INCREMENT();
    dvariable mnMltInc, r, mu;
...
FUNCTION dvar_vector calc_like_gamma_pwrlawalt_growth(const dvector dMoltInc, const dvector vSizes, const ivector iSex, int igrow)
  if ( vSizes.indexmin() != iSex.indexmin() || vSizes.indexmax() != iSex.indexmax() )
    { cerr << "indices don't match..." << endl; ad_exit(1); }
  RETURN_ARRAYS_INCREMENT();
...
FUNCTION calc_initial_numbers_at_length
  ...
  switch( bInitializeUnfished ){
    ...
    case FISHEDEQN:                                        ///> Steady-state fished conditions
      ...
      break;
    case EQMDEVS:                                          //> Equilibrium + penalised deviations
      log_initial_recruits = logRini;
      break;
  ...
  switch( bInitializeUnfished ){
    ...
    case FREEPARSSCALED:                                 ///> Free parameters (revised)
     ...
     break;
    case EQMDEVS:                                 ///> Equilibrium x exp(deviations)
...
// --------------------------------------------------------------------------------------------------------------------------------------
   ...
   */
FUNCTION dvar_vector getNumbersAtLengthByCase(const int ig, const int i, const int j, const int Case)
  RETURN_ARRAYS_INCREMENT();
  int h = isex(ig);
  int m = imature(ig);
  int o = ishell(ig);
...
// --------------------------------------------------------------------------------------------------------------------------------------
   ...
   * @param Case - flag indicating type of numbers-at-length array to update
   */
FUNCTION void updateNumbersAtLengthByCase(const dvar_matrix& next, const int i, const int j, const int Case)
  //if (verbose>3){
...
FUNCTION void update_population_numbers_at_length_season_new(const int i, const int j, const int Case)
...
//  for (int i = syr; i <= nyrRetro; i++ ){  //--TODO: start at syr (already calculated in 'initial' with no dev) and overwrite?
//    for (int h = 1; h <= nsex; h++ ){
//      if (verbose>4) if ((verbose>3)&&(NfunCall<2)) cout<<"---looping for recruits: i(yr) h(sex): "<<i<<" "<<h<<endl;
//      if ( bInitializeUnfished == UNFISHEDEQN ){
//        totrecruits(i) = float(nsex)*mfexp(logR0)*mfexp(rec_dev(i));
//      } else {
//        totrecruits(i) = float(nsex)*mfexp(logRbar(i))*mfexp(rec_dev(i));
//      }
//      if (nsex == 1) {
//        recruits(h)(i) = totrecruits(i);
//      } else if (nsex == 2) {
//        // This splits recruitment out proportionately into males and females
//        if (h == MALES)   recruits(h)(i) = totrecruits(i) * 1.0 / (1.0 + mfexp(-logit_rec_prop(i)));
//        if (h == FEMALES) recruits(h)(i) = totrecruits(i) * (1.0 - 1.0 / (1.0 + mfexp(-logit_rec_prop(i))));
//      }
//      //---------------------------------------------
//      if ((verbose>4) && NfunCall<2){
//        cout<<recruits(h)(i)<<endl;
//        if (std::isnan(value(recruits(h)(i)))) {
//          cout << "In update_population_numbers_at_length: recruits has NaNs for year, sex " << i << " " << h << endl;
//          exit(-1);
//        }
//      }
//    }//--h
...
// ----------------------------------------------------------------------------------------------
  ...
  **/
FUNCTION calc_predicted_capture_abdbio
  int debug = 0;
...
//   /**
//    * @brief Calculate predicted relative abundance and residuals (original--does not handle blocks for q)
...
//    * 
//   **/
// FUNCTION calc_relative_abundance_old
//   int unit;
//   dvar_vector sel(1,nclass);                                         ///> capture selectivity
...
//                 if ((h==h1) || (h== UNDET_SEX)) {  //--TODO: h will never = UNDET_SEX. Should this be h1==UNDET_SEX??
...
//             if (m1 ==IMMATURE) {
//               //=========insert case for immature only crab...if anyone would actually do this (Tanner does this for immature females!!)============  
...
//     if (verbose>3) {
//       echoinput << "#--Calculating predicted cpue and residuals for srv_idx k = " << k << endl;
//       echoinput << "srv_idx(k)  row(jj)   year(i)  dSurveyData(jj,0)   fleet  sex   maturity  SurveyQT    obs_cpue     pre_cpue     res_cpue    sel " << endl;
//     }
//     for ( int jj = 1; jj <= nSurveyRows; jj++ ){
//       if (dSurveyData(jj,0) == k){
//         if (((dSurveyData(jj,1) <= nyrRetro) || ((dSurveyData(jj,1) == nyrRetro+1) && (dSurveyData(jj,2) == 1))) && (dSurveyData(jj,1) >= syr)) {
...
//           if(verbose>3) {
//             echoinput << k << " " << jj << " " << i << " " << " " << dSurveyData(jj,0) << " " << dSurveyData(jj,3) << " " << dSurveyData(jj,4) << " " << dSurveyData(jj,5) << " ";
//             echoinput << SurveyQT(k,i) << "  " << obs_cpue(jj) << " " << pre_cpue(jj) << " " << res_cpue(jj) << " ";
...
FUNCTION recruitment_likelihood
  ...
  if ( active(logit_rec_prop_est) )                                  ///> Sex-ratio devs
   nloglike(4,3) = dnorm(logit_rec_prop_est, 1.0);
...
FUNCTION calc_objective_function
  ...
  // 5) Initial-abundance deviations from the equilibrium (Rceattle initMode 2,
...
FUNCTION CreateOutput
  ...
  OutFile1 << "--Penalties: list" << endl;
    ...
    OutFile1 << "5. Init_devs: " << nlogPenalty(5) << " " << Penalty_emphasis(5) << " " << nlogPenalty(5)*Penalty_emphasis(5) << endl;
...
FUNCTION WriteFiles
  ...
  OutInpFile1 << setw(4) << setfixed()<< int(model_controls(18)) << " # recruitment size distribution option (0: standard way; 1: Tanner crab approach)" << endl;
  OutInpFile1 << setw(4) << setfixed()<< int(model_controls(19)) << " # Set to 1 to copy growth from males to females)" << endl;
  OutInpFile1 << endl;
...
// --------------------------------------------------------------------------------------------------
 ...
 double GenJitter(int JitterType, int IsPin, double Initial1, double lower, double upper, int Phase, double sdJitter, dvector& rands, dvector& randu, dvariable Initial2)
  {
   double ParValue,eps;
   double Initial;
   
   if (IsPin==0) Initial = Initial1;
   if (IsPin==1) Initial = value(Initial2);
...
//                    not `nsel_patterns`.
// 2024-10-22 ** WTS. (GMACS Version 2.20.17) Explicitly set n_deviations = 0 after declaration. 
//                    Added ECHOs for n_deviations at various stages of counting as diagnostics.
...
// 2025-03-31 ** WTS. (GMACS Version 2.20.22) Added gamma distribution likelihood type (GROWTHINC_DATA_GAMMA) for non-tagging molt increment 
//                    data so likelihood is consistent with size transition matrices. Previous normal likelihood based on data CVs is still available. 
...
//                    Reformated some code sections for better readability and to replace tabs with spaces. Added option and function to replicate 
//                    Tanner crab mean molt increment calculations.
// 2025-04-01 ** WTS. (GMACS Version 2.20.23) Added function CreateOutput_OldFormat() to write output to  "Gmacsall.out-old", "gmacs.rep-old", 
//                    "personal.rep-old", "simdata.out-old", and "Gmacsall.std-old" in the 2.20.06 format (before the output was niceified for R). Was
//                    supposed to help Cody by providing "old"-style output, but he was working with a really old (2.01.M.02 **CSS** 2023-04-12) 
//                    version of GMACS. However, this may benefit users who work with the gmr R package or other code based on the older format.
// 2025-04-01 ** WTS. (GMACS Version 2.20.24) Added macro var ZEROPOP (=5)option to initialize numbers-at-size in start year at 0. The 
//                    logN0 is set to -1.0e3.
// 2025-04-07 ** WTS. Fixed incorrectly listed sex in gmacs_in.ctl and gmacs_out.ctl files for probability of maturing when two sex are specified. 
//                    Fixed problem in get_all_sdnr_MAR when d3_res_size_comps(k) was missing (nSizeComps(k=0) because size comps were "extended").
//                    Distinguished between tagging-related (nloglike(5)) and non-tagging-related growth data likelihoods (now nloglike(6)) in code and 
//                    Gmacsall.out file. Non-tagging growth likelihoods are now output by indivdual observation in Gamcsall.out as a dataframe ("growth_data").
...
//                    Input size comps in "new" format now respect sex-specific numbers of columns: all input columns are repeated to gmacs_data (file "gmacs_in.dat"), 
//                    but only the sex-specific number of columns is kept in the internal arrays.
...
// 2025-04-16 ** WTS. Added commandline input option "-StopAfterFnCall xx" to override StopAfterFnCall setting in gmacs.dat file with value of "xx".
//                    Converted "Double normal" as output label to "Double_normal" to be consistent w/ other output labels and facilitate parsing.
//                    Added ascending normal selectivity function code to selex.hpp, SELEX_ASCNORM flag (14) to select it, and associated in/out ctl code .
// 2025-04-17 ** WTS. Incremented version number to 2.20.25. Reformatted some code related to ctl-file q input specification, added survey info to 
//                    gmacs_in.ctl and OutInpFile1 file output for q specification.
// 2025-04-21 ** WTS. Incremented version number to 2.20.26. Found issue with RW parameters for selectivity when RW block group has multiple blocks. Fixed 
//                    for selectivity but may be an issue for other processes when RW parameters are specified. 
//                    Also tracked down what turned out to be an uninitialized array problem (nSizeCompRows) that sent Windows runs off into NeverNeverLand.
//                    Also added logic to skip calculating length likelihoods for the size comps that are the "extended" part of other comps.
// 2025-04-24 ** WTS. Lot of revision to enable blocks to work with catchability (q) parameters--many bits of code were based on assumption that q's 
//                    not blocked or time-varying. Refer to "Relative Abundance Indices" (RAI's) and id's for 
//                    such rather than "surveys" because the latter was confused between survey indices and catchability parameters. Rows for RAI's in the data 
//                    file need to be appended with the RAI id in order to match changes in q id with RAI id (might not be strictly necessary as q id is given 
//                    and qToSurv(iq) gives the id of the associated non-mirrored RAI; however, doesn't give RAI's for mirrored q's).
//                    Required removing CreateOutput_OldFormat. Incremented to version 2.20.27.
// 2025-04-24 ** WTS. Added foffdevs_phz vector to set phases for female offset F devs (log_fdovs. phases had been set by foff_phz, 
//                    but needed to be turned "off" when no female F devs were defined.) Changed logN0 to -100 when ZEROPOP option is selected (had been -1000).
//                    Incremented version to 2.20.28.
// 2025-05-27 ** WTS. Added input for 'recZ_flag' to CTL file as model_controls(18). Flag determines parameterization for recruitment size distribution in 
//                    calc_recruitment_size_distribution (0=standard approach; 1=Tanner approach). Incremented version to 2.20.29.
// 2025-05-28 ** WTS. Reassigned SELEX_UNIFORM0 and SELX_UNIFORM1 values from (6,5) to (5,6) to agree with ctl file comments. 
//                    Kept version at 2.20.29 since this has not been pushed to "main" yet. 
//                    Changed useage of RW parameter definition value FOR SELECTIVITY PARAMETERS ONLY. Was 0: RW devs off; >0: RW devs on. 
//                    Now, for selectivity parameters only,: 0: RW devs off; 1: RW devs on w/ newpar = refpar*exp(dev) [as previously]; 
//                    2: newpar = refpar + dev [new option--way to go for many parametric selectivity]. Changed description for selectivity sections 
//                    of output ctl files. For other parameter types, the behavior is the same as previously (TODO: check if this behavior should be changed, as well).
// 2025-05-29 ** WTS. Added a check on the size of dataframe read in as dSurveyData when using original format to assure RAI_id column has been added. 
//                    After dSurveyData is read in, an integer 'chk' is read from the input data file. If this is not equal to 999, then an error is printed and the program exits. 
//                    Also, revised some information output to various files to clarify things a bit more. Finally, corrected an error reading parameter information
//                    for time-blocked "extra" growth parameters (did not affect non-time-blocked growth parameters).
// 2025-06-04 ** WTS. Cleaned up code in update_population_numbers_at_length related to recruitment, with note for further changes. 
//                    Added note in calc_initial_numbers_at_length with reminder to change code if logR0, etc are redefined to apply to 
//                    totrecruits rather than recruits(h).
// 2025-06-09 ** WTS. Renamed gmacs.rep1 output matrix from "Fully-selected_FM_by_season_sex_and_fishery" to "Fully-selected_capture_rate_by_season_sex_and_fishery" to
//                    better describe the content (ft). Note that F(h,i,j)=ft(h,i,j)*vul is the size-specific rate of fishing mortality 
//                    (vul incorporates both retention mortality and discard mortality) while F2(h,i,j) = ft(h,i,j)*sel is the size-specific fishery **capture** rate.
// 2025-06-10 ** WTS. Added pre_capbio array and calc_predicted_capture_biomass function to facilitate export of predicted capture biomass by fishery in  
//                    CreateOutput as Predicted_capture_biomass-at-size. Fixed inconsistency in how Z and Z2 were assigned as tempZ1 in calc_predicted_catch. Lots of reformatting. 
//                    Added output to gmacs.rep1 for predicted fishery capture abundance and biomass (pre_capabd, pre_capbio) by year, fleet, sex, and size from 
//                    (new function) calc_predicted_capture_abdbio. Updated version to 2.20.30.
...
// 2025-07-31 ** AEP. Allowed for more general environmental drivers for M
// 2026-12-30 ** AEP. Allowed for more general environmental drivers for mean recruitment. Incremented version to 2.20.32.
//                    NOTE: THIS CHANGES the specification for the "MAIN PARS"/recruitment section of the ctl file: "phase" is specified in the column 
//                    after Prior_2 and additional columns Block, Block_fn Env_L, Env_vr, RW, RW_Blk, and RW_Sigma are now required.
//                    NOTE: you will also need to add a line to the ctl file after the Block_Groups specification to specify the number of environmental treatments.
// 2026-01-05 ** AEP. Found a print out bug
// 2026-01-14 ** AEP. Incremented version to 2.20.32a.
// 2026-01-14 ** TJ.  Allowed for spr_lambda to be equal to 1 for full contribution of both sexes when ctl input is negative. Incremented version to 2.20.32b.
// 2026-01-14 ** AEP. Added the IsTyler flag as model_controls(19) which forces female growth to match that for males
// 2026-01-15 ** WTS. Updated version to 2.20.33.
// 2026-01-15 ** WTS. Added output for isTyler/model_controls(19) to gmacs_stl, echoinput, and OutInpFile1. 
//                    Incremented version to 2.20.33a.
// 2026-01-15 ** AEP. Correct selectivity deviance parameter pointer. Revised the jitter function so that the jittered value is centered from a value from a PIN file.
//                    Incremented version to 2.20.34.

// To-do list
// 1 - revise size of log_vn vector (estimated multinomial sample sizes). currently it's = max(iCompAggregator), but it possibly makes more sense 
//     to have it equal the number of aggregated size comps
